...
class ImageComparisonTool(QMainWindow):
    ...
    def __init__(self):
        ...
        print(
            f"当前日期时间: {QDateTime.currentDateTimeUtc().toString('yyyy-MM-dd HH:mm:ss')}"
        )
    ...
    def setup_ui(self):
        ...
        original_layout.addWidget(
            QLabel("原始图像 (使用鼠标滚轮缩放，拖动平移，或使用滚动条)")
        )
        ...
        translated_layout.addWidget(
            QLabel("翻译图像 (使用鼠标滚轮缩放，拖动平移，或使用滚动条)")
        )
    ...
    def save_current_annotation(self, annotation_text=None):
        ...
        try:
            ...
            if annotation_text:
                ...
                annotation_short = "".join(
                    c for c in annotation_text[:20] if c.isalnum() or c in " _-"
                )
            ...
            if self.original_view.annotations:
                orig_image = QImage(
                    self.original_scene.sceneRect().size().toSize(),
                ...
                painter.end()
                orig_image.save(
                    os.path.join(self.annotation_folder, f"orig_{new_filename}")
                )

            # 保存翻译图像带标注
            if self.translated_view.annotations:
                trans_image = QImage(
                    self.translated_scene.sceneRect().size().toSize(),
                    QImage.Format_ARGB32,
                )
            ...
            # 如果两个视图都没有标注，给出提示
            if (
                not self.original_view.annotations
                and not self.translated_view.annotations
            ):
    ...
    def reset_views(self):
        ...
        self.original_view.fitInView(
            self.original_scene.sceneRect(), Qt.KeepAspectRatio
        )
        self.translated_view.fitInView(
            self.translated_scene.sceneRect(), Qt.KeepAspectRatio
        )

    def select_image_folders(self):
        """选择包含原始图像和翻译图像的文件夹"""
        # 选择原始图像文件夹
        original_folder = QFileDialog.getExistingDirectory(
            self, "选择原始图像文件夹", ""
        )
        ...
        # 选择翻译图像文件夹
        translated_folder = QFileDialog.getExistingDirectory(
            self, "选择翻译图像文件夹", ""
        )
    ...
    def find_image_pairs(self):
        ...
        original_images = [
            f
            for f in os.listdir(self.original_folder)
        ...
        for filename in sorted(matching_filenames):
            ...
            if orig_img.size() == trans_img.size():
                ...
                print(
                    f"警告: 图像 {filename} 的尺寸不匹配，原始尺寸: {orig_img.size()}, 翻译尺寸: {trans_img.size()}"
                )
    ...
    def load_current_image_pair(self):
        ...
        try:
            ...
            self.original_scene.setSceneRect(
                0, 0, original_pixmap.width(), original_pixmap.height()
            )
            ...
            self.translated_scene.setSceneRect(
                0, 0, translated_pixmap.width(), translated_pixmap.height()
            )
            ...
            self.status_label.setText(
                f"当前图像: {filename} ({self.current_index + 1}/{len(self.image_pairs)})"
            )
            ...
            print(
                f"翻译图像尺寸: {translated_pixmap.width()}x{translated_pixmap.height()}"
            )
    ...
    def update_navigation(self):
        ...
        self.next_button.setEnabled(
            has_images and self.current_index < len(self.image_pairs) - 1
        )
    ...
    def export_annotated_images(self):
        ...
        try:
            ...
            # 处理所有图像对
            for i, (original_path, translated_path, filename) in enumerate(
                self.image_pairs
            ):
                # 切换到当前图像对
